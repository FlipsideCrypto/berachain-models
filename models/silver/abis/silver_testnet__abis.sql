...
WITH override_abis AS (
    ...
    SELECT
        ...
        1 AS priority
    FROM
        {{ ref('silver_testnet__override_abis') }}
    WHERE
...
verified_abis AS (
    SELECT
        ...
        2 AS priority
    FROM
        {{ ref('silver_testnet__verified_abis') }}
    WHERE
        abi_source = 'routescan'
...
bytecode_abis AS (
    SELECT
        ...
        3 AS priority
    FROM
        {{ ref('silver_testnet__bytecode_abis') }}
    WHERE
        1 = 1
...
FROM
    priority_abis p
    LEFT JOIN {{ ref('silver_testnet__created_contracts') }}
